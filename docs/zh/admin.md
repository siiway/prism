...
| 取值                                               | 效果                                                                       |
| -------------------------------------------------- | -------------------------------------------------------------------------- |
| 未设置（或无法识别的值）                           | 完全访问 —— 浏览、编辑行、执行任意语句                                     |
| `read-only` / `readonly` / `read`                  | 仅浏览与 `SELECT`。一切写入都会被拒绝，包括显式带 `allow_write` 的请求 —— 调用方无法翻越运营者的设置 |
| `off` / `false` / `0` / `no` / `disabled` / `none` | 界面消失。端点返回 404，标签页不再显示                                     |

默认是完全访问，因为部署实例的运营者本就是这个功能的受众，数据库本来就属于他们。不愿承担这份风险的运营者可以把它关小；该设置每次请求都会读取，因此改动只需一次 `wrangler deploy`。
