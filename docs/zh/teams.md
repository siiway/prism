...
**站点管理员**对每个团队都拥有所有者级权限 —— 以站点身份行使，即便对自己所属或拥有的团队也是如此，添加成员时还可以覆盖团队自身的加入门槛。团队页会以横幅予以说明，其操作会带 `site_admin: true` 记入该团队的审计日志。管理员可**切换到普通视图**改以自己的成员身份行事。详见 [管理员 → 站点管理员对所有团队的权限](admin.md#站点管理员对所有团队的权限)。
