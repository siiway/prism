...
```text
浏览器 → /api/*        → Worker（Hono 路由）
浏览器 → /             → Cloudflare Assets → index.html
浏览器 → /some/route   → Cloudflare Assets → index.html（SPA 回退）
浏览器 → /.well-known/ → Worker
```
