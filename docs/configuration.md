...
| `KV_CONSOLE`        | No       | Availability of the key–value browser in the same panel. Same values as `D1_CONSOLE`. **Unset follows `D1_CONSOLE`** — the two are windows onto the same instance's storage, so turning the database console off is a statement about both unless this one says otherwise. |
