...
app.get('/me', async (c) => {
  ...
  const totp = await c.env.DB.prepare('SELECT enabled FROM totp_secrets WHERE user_id = ?')
    .bind(user.id)
    .first<{ enabled: number }>();
  const passkeyCount = await c.env.DB.prepare('SELECT COUNT(*) as n FROM passkeys WHERE user_id = ?')
    .bind(user.id)
    .first<{ n: number }>();
...
app.post('/me/change-password', async (c) => {
  ...
  if (row.password_hash) {
    if (!body.current_password) return c.json({ error: 'current_password required' }, 400);
...
app.post('/me/avatar', async (c) => {
  ...
  if (file.size > 2 * 1024 * 1024) return c.json({ error: 'Avatar must be < 2MB' }, 400);
...
app.delete('/me', async (c) => {
  ...
  if (body.confirm !== 'DELETE') return c.json({ error: 'Confirm deletion by sending confirm="DELETE"' }, 400);
