...
app.post('/', async (c) => {
  ...
  if (!domainRegex.test(body.domain)) return c.json({ error: 'Invalid domain format' }, 400);

  const domain = body.domain.toLowerCase().trim();

  const existing = await c.env.DB.prepare('SELECT id FROM domains WHERE user_id = ? AND domain = ?')
    .bind(user.id, domain)
    .first();
...
app.post('/:id/verify', async (c) => {
  ...
  const row = await c.env.DB.prepare('SELECT * FROM domains WHERE id = ? AND user_id = ?')
    .bind(id, user.id)
    .first<DomainRow>();
