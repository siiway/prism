...
app.get('/config', async (c) => {
  ...
  const safeConfig = { ...config, captcha_secret_key: '***', github_client_secret: '***', google_client_secret: '***', microsoft_client_secret: '***', discord_client_secret: '***', email_api_key: '***' };
...
app.patch('/config', async (c) => {
  ...
  const allowed = new Set([
    ...
    'session_ttl_days', 'access_token_ttl_minutes', 'refresh_token_ttl_days',
    'github_client_id', 'github_client_secret',
    'google_client_id', 'google_client_secret',
    'microsoft_client_id', 'microsoft_client_secret',
    'discord_client_id', 'discord_client_secret',
    'email_provider', 'email_api_key', 'email_from',
    'custom_css', 'accent_color',
  ]);
  ...
  await logAudit(c.env.DB, c.get('user').id, 'admin.config.update', 'site_config', null, updates, getIp(c));
...
app.get('/users/:id', async (c) => {
  ...
  const [apps, connections, sessions] = await Promise.all([
    c.env.DB.prepare('SELECT id, name, client_id, is_active, created_at FROM oauth_apps WHERE owner_id = ?')
      .bind(id)
      .all(),
    c.env.DB.prepare('SELECT provider, connected_at FROM social_connections WHERE user_id = ?')
      .bind(id)
      .all(),
    c.env.DB.prepare('SELECT id, user_agent, ip_address, created_at, expires_at FROM sessions WHERE user_id = ?')
      .bind(id)
      .all(),
  ]);
  ...
  return c.json({
    user: {
      ...
      role: user.role, email_verified: user.email_verified === 1,
      is_active: user.is_active === 1, created_at: user.created_at,
    },
    apps: apps.results,
...
app.patch('/users/:id', async (c) => {
  ...
  const id = c.req.param('id');
  const body = await c.req.json<{ role?: 'admin' | 'user'; is_active?: boolean; email_verified?: boolean }>();
  ...
  if (body.email_verified !== undefined) {
    updates.push('email_verified = ?'); values.push(body.email_verified ? 1 : 0);
  }
  ...
  await logAudit(c.env.DB, admin.id, 'admin.user.update', 'user', id, body, getIp(c));
...
app.delete('/users/:id', async (c) => {
  ...
  await logAudit(c.env.DB, admin.id, 'admin.user.delete', 'user', id, {}, getIp(c));
...
app.get('/apps', async (c) => {
  ...
  const [apps, count] = await Promise.all([
    ...
    c.env.DB.prepare('SELECT COUNT(*) as n FROM oauth_apps').first<{ n: number }>(),
  ]);
...
app.patch('/apps/:id', async (c) => {
  const admin = c.get('user');
  const id = c.req.param('id');
  const body = await c.req.json<{ is_verified?: boolean; is_active?: boolean }>();

  const app = await c.env.DB.prepare('SELECT id FROM oauth_apps WHERE id = ?').bind(id).first();
  if (!app) return c.json({ error: 'App not found' }, 404);

  const updates: string[] = [];
  const values: unknown[] = [];
  if (body.is_verified !== undefined) { updates.push('is_verified = ?'); values.push(body.is_verified ? 1 : 0); }
...
app.get('/stats', async (c) => {
  const [userCount, appCount, domainCount, tokenCount] = await Promise.all([
    ...
    c.env.DB.prepare('SELECT COUNT(*) as n FROM oauth_apps').first<{ n: number }>(),
    c.env.DB.prepare('SELECT COUNT(*) as n FROM domains WHERE verified = 1').first<{ n: number }>(),
    c.env.DB.prepare('SELECT COUNT(*) as n FROM oauth_tokens WHERE expires_at > ?').bind(Math.floor(Date.now() / 1000)).first<{ n: number }>(),
  ]);
  return c.json({
    users: userCount?.n ?? 0,
    apps: appCount?.n ?? 0,
...
app.get('/audit-log', async (c) => {
  ...
  const count = await c.env.DB.prepare('SELECT COUNT(*) as n FROM audit_log').first<{ n: number }>();
...
async function logAudit(
  ...
  await db
    ...
    .bind(randomId(), userId, action, resourceType, resourceId, JSON.stringify(metadata), ip, Math.floor(Date.now() / 1000))
    .run();
}
