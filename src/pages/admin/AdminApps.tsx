              ...
              <TableRow key={app.id}>
                <TableCell>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {app.icon_url && <img src={app.icon_url} alt={app.name} width={24} height={24} style={{ borderRadius: 4 }} />}
                    <div>
                      <Text weight="semibold" block>{app.name}</Text>
                      <Text size={200} style={{ color: tokens.colorNeutralForeground3 }}>{app.description?.slice(0, 40)}</Text>
                    </div>
                  </div>
                </TableCell>
                <TableCell>@{app.owner_username}</TableCell>
                <TableCell style={{ fontFamily: 'monospace', fontSize: '12px' }}>{app.client_id}</TableCell>
                <TableCell>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Badge
                      color={app.is_verified ? 'success' : 'subtle'}
                      appearance="filled"
                      icon={app.is_verified ? <ShieldCheckmarkRegular /> : undefined}
                    >
                    ...
                    <Button
                      size="small"
                      appearance="subtle"
                      onClick={() => handleToggleVerified(app.id, app.is_verified)}
                    >
                ...
                <TableCell>{new Date(app.created_at * 1000).toLocaleDateString()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
