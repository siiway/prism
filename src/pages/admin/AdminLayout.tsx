...
export function AdminLayout() {
  ...
  const currentTab = TABS.find((t) => pathname === t.value || (t.value !== '/admin' && pathname.startsWith(t.value)))?.value ?? '/admin';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        ...
        <br />
        <Text style={{ color: tokens.colorNeutralForeground3 }}>Manage your Prism platform.</Text>
      </div>

      <TabList selectedValue={currentTab} onTabSelect={(_, d) => navigate(d.value as string)}>
        {TABS.map((t) => (
