...
export function AdminSettings() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      {message && <MessageBar intent={message.type === 'success' ? 'success' : 'error'}>{message.text}</MessageBar>}
        ...
        <div className={styles.card}>
          ...
          <div className={styles.form}>
            ...
            <Field label="Site Icon URL">
              <Input value={get('site_icon_url') ?? ''} onChange={(e) => set('site_icon_url', e.target.value || null)} placeholder="https://..." />
            </Field>
            <Switch
          ...
          <div className={styles.form}>
            <div className={styles.row}>
              <Field label="Session TTL (days)">
                <Input type="number" value={String(get('session_ttl_days') ?? 30)} onChange={(e) => set('session_ttl_days', parseInt(e.target.value))} />
              </Field>
              <Field label="Access Token TTL (minutes)">
                <Input type="number" value={String(get('access_token_ttl_minutes') ?? 60)} onChange={(e) => set('access_token_ttl_minutes', parseInt(e.target.value))} />
              </Field>
            </div>
            <div className={styles.row}>
              <Field label="Refresh Token TTL (days)">
                <Input type="number" value={String(get('refresh_token_ttl_days') ?? 30)} onChange={(e) => set('refresh_token_ttl_days', parseInt(e.target.value))} />
              </Field>
              <Field label="Domain Re-verify (days)">
                <Input type="number" value={String(get('domain_reverify_days') ?? 30)} onChange={(e) => set('domain_reverify_days', parseInt(e.target.value))} />
              </Field>
            </div>
          </div>
        </div>
          ...
          <div className={styles.form}>
            <Field label="Provider">
              <Dropdown
                ...
                onOptionSelect={(_, d) => set('captcha_provider', d.optionValue)}
              >
                <Option value="none">None</Option>
                <Option value="turnstile">Cloudflare Turnstile</Option>
                <Option value="hcaptcha">hCaptcha</Option>
                <Option value="recaptcha">Google reCAPTCHA v3</Option>
                <Option value="pow">Proof of Work (no server key needed)</Option>
              </Dropdown>
            </Field>
            {get('captcha_provider') !== 'none' && get('captcha_provider') !== 'pow' && (
              <>
                <Field label="Site Key">
                  <Input value={get('captcha_site_key') ?? ''} onChange={(e) => set('captcha_site_key', e.target.value)} />
                </Field>
                <Field label="Secret Key">
                  <Input type="password" value={get('captcha_secret_key') ?? ''} onChange={(e) => set('captcha_secret_key', e.target.value)} />
                </Field>
              </>
            )}
            {get('captcha_provider') === 'pow' && (
              <Field label="PoW Difficulty (leading zero bits, 16-28 recommended)">
                <Input type="number" value={String(get('pow_difficulty') ?? 20)} onChange={(e) => set('pow_difficulty', parseInt(e.target.value))} />
              </Field>
            )}
          </div>
        </div>
      ...
      {tab === 'social' && (
        <div className={styles.card}>
          ...
          <div className={styles.form}>
            {[
              { label: 'GitHub', idKey: 'github_client_id' as const, secretKey: 'github_client_secret' as const },
              { label: 'Google', idKey: 'google_client_id' as const, secretKey: 'google_client_secret' as const },
              { label: 'Microsoft', idKey: 'microsoft_client_id' as const, secretKey: 'microsoft_client_secret' as const },
              { label: 'Discord', idKey: 'discord_client_id' as const, secretKey: 'discord_client_secret' as const },
            ].map(({ label, idKey, secretKey }) => (
              <div key={label}>
                <Text weight="semibold" block style={{ marginBottom: 8 }}>{label}</Text>
                <div className={styles.row}>
                  <Field label="Client ID">
                    <Input value={get(idKey) ?? ''} onChange={(e) => set(idKey, e.target.value)} />
                  </Field>
                  <Field label="Client Secret">
                    <Input type="password" value={get(secretKey) ?? ''} onChange={(e) => set(secretKey, e.target.value)} placeholder="(unchanged)" />
                  </Field>
                </div>
              </div>
            ))}
        ...
        <div className={styles.card}>
          ...
          <div className={styles.form}>
            ...
            {get('email_provider') !== 'none' && (
              <Field label="API Key">
                <Input type="password" value={get('email_api_key') ?? ''} onChange={(e) => set('email_api_key', e.target.value)} placeholder="(unchanged)" />
              </Field>
            )}
      ...
      <div className={styles.actions}>
        <Button appearance="primary" onClick={handleSave} disabled={saving || Object.keys(localConfig).length === 0}>
