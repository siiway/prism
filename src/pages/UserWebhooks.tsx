// User webhook management — create, configure, and monitor personal webhooks
...
const USER_EVENTS = [
  ...
  { value: "app.created", group: "webhooks.groupApps" },
  { value: "app.updated", group: "webhooks.groupApps" },
  { value: "app.deleted", group: "webhooks.groupApps" },
  { value: "domain.added", group: "webhooks.groupDomains" },
  { value: "domain.verified", group: "webhooks.groupDomains" },
  { value: "domain.deleted", group: "webhooks.groupDomains" },
  { value: "profile.updated", group: "webhooks.groupProfile" },
];
...
function EventSelector({
  ...
  const groups = [...new Set(USER_EVENTS.map((e) => e.group))];
...
function WebhookRow({ wh }: { wh: Webhook }) {
  ...
  const deliveries = useQuery({
    queryKey: ["user-webhook-deliveries", wh.id],
    queryFn: () => api.listUserWebhookDeliveries(wh.id),
    enabled: expanded,
  });

  const remove = useMutation({
    mutationFn: () => api.deleteUserWebhook(wh.id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["user-webhooks"] }),
  });

  const test = useMutation({
    mutationFn: () => api.testUserWebhook(wh.id),
    onSuccess: (d) => {
      setTestResult({ success: d.success, status: d.status });
      qc.invalidateQueries({ queryKey: ["user-webhook-deliveries", wh.id] });
    },
  });
