...
import { FluentProvider, webLightTheme, webDarkTheme, createLightTheme, createDarkTheme } from '@fluentui/react-components';
...
export function ThemeProvider({ children }: ThemeProviderProps) {
  const { data: siteConfig } = useQuery({ queryKey: ['site'], queryFn: api.site, staleTime: 60_000 });
  ...
  return (
    <FluentProvider theme={theme} style={{ minHeight: '100vh', background: 'var(--colorNeutralBackground1)' }}>
      {children}
    </FluentProvider>
  );
}
